...
fast.fsr.ps<-function(x,y,gamma0=.05,delta=1,center=T,stdx=T,rescaley=T,maxsteps=kt,quad=F,print=T,digits=4){
require(MASS)

#This function is used to get an average rate of entry over |beta|<delta

C.alpha=function(alpha,SD,delta){
# finds C(alpha) for UMPU test with known SD
fc=function(c){pnorm(-c)+pnorm(-c-2*delta/SD)-alpha}
if(alpha>1e-11) {

CA=uniroot(f=fc,lower=qnorm(1-alpha)-1,upper=qnorm(1-alpha)+5)
CA
...
#Rescale covariates using standard deviation by default
if(stdx)
{
meanx <- drop(one %*% x)/n
...
sxx<-invxpx*sigmahat
t1a<-(beta[i+1]-delta)/(sqrt(sxx[i+1,i+1]))
t1b<-(-beta[i+1]-delta)/(sqrt(sxx[i+1,i+1]))
t2a<-(beta[i+1]+delta)/(sqrt(sxx[i+1,i+1]))
t2b<-(-beta[i+1]+delta)/(sqrt(sxx[i+1,i+1]))
df=n-i-1
pval1a<-1-pt(t1a,df)
pval1b<-pt(t1b,df)
pval2a<-pt(t2a,df)
...
#put calpha stuff here
sd<-sqrt(sxx[i+1,i+1])



if(maxpval.step[i+1]<1) {
  if(maxpval.step[i+1]>1e-11) {
    calpha<-C.alpha(maxpval.step[i+1],sqrt(sxx[i+1,i+1]),delta)$root
  }
  else calpha<-8.209536


  talpha<-(calpha*sd/delta+2)*pnorm(-calpha-2*delta/sd)-
  (calpha*sd/delta)*pnorm(-calpha)+(sd/delta)*dnorm(-calpha)-
  (sd/delta)*dnorm(-calpha-2*delta/sd)
...
ghat.step[i+1]<-(totcand.step[i+1]*talpha)/(1+totinmodel)
ghigh.step[i]<-(totcand.step[i]*talpha)/(totinmodel)
if(i==maxstep) ghigh.step[i+1]<-(totcand.step[i+1]*1)/(1+totinmodel)

#I think it may be difficult to estimate the true alpha
#Would need to invert the talpha function
...
if(print){
	#print(paste("Estimated alpha:", round(alpha.fsr,digits))) 
...
return(list(info.step=info.step,rsquare.fsr=rsquare.fsr,model.fsr=model.fsr,size=fsrselect))
